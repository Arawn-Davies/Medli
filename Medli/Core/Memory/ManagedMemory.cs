...
namespace Medli.Core
{
    public unsafe class ManagedMemoryBlock
    {
        ...
        public ManagedMemoryBlock(UInt32 size, short alignment)
        ...
        public ManagedMemoryBlock(UInt32 size, short alignment, bool align)
        {
            memory = new short[size + alignment - 1];
            fixed (short* bodystart = memory)
            {
