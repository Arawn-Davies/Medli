...
{
	public class SVGA_Graphics
	{
		private static byte[] font;
		public static Medli.Hardware.Drivers.Video.SVGAIIDriver svga;
		private static uint[] pallete = new uint[16];

		public SVGA_Graphics()
		{
			...
			font = Read_font();
			svga = new Hardware.Drivers.Video.SVGAIIDriver();
		...
		{
			int p = 0;
			Kernel.xConsole.Y += height / 15;
			for (ushort x = 0; x < Length; x++)
			{
				for (ushort y = 0; y < height; y++)
				{
					if (y < (height - 2) || x != (Kernel.xConsole.Width - 2))
			...
			for (int cy = 0; cy < 16; cy++)
			{
				for (byte cx = 0; cx < 8; cx++)
				{
					if (getb(font[p + cy], cx + 1))
						svga.SetPixel((ushort)((9 * (Kernel.xConsole.X)) + (9 - cx)), (ushort)((16 * (Kernel.xConsole.Y)) + cy), pallete[VMWareSVGxConsole.foreground]);
					else
						svga.SetPixel((ushort)((9 * (Kernel.xConsole.X)) + (9 - cx)), (ushort)((16 * (Kernel.xConsole.Y)) + cy), 0);
				}
			}
		}
