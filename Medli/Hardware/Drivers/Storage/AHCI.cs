...
namespace Medli.Hardware.Drivers.BlockDevice
{
    public class AHCI
    {
        ...
        internal static PCIDevice xDevice = PCI.GetDeviceClass(ClassID.MassStorageController,
                                                                   SubclassID.SATAController,
                                                                   ProgramIF.SATA_AHCI);

        private static List<Ports.StoragePort> mPorts = new List<Ports.StoragePort>();
        ...
        public AHCI(PCIDevice aAHCIDevice)
        {
            ...
            foreach(Ports.StoragePort xPort in mPorts)
            {
        ...
        public static void Wait(int microsecondsTimeout)
        {
            ...
            {
				// Random IOPort
				xVoid = Cosmos.Core.Global.BaseIOGroups.TextScreen.Data1.Byte;
                xVoid = Cosmos.Core.Global.BaseIOGroups.TextScreen.Data1.Byte;
                xVoid = Cosmos.Core.Global.BaseIOGroups.TextScreen.Data1.Byte;
                xVoid = Cosmos.Core.Global.BaseIOGroups.TextScreen.Data1.Byte;
                xVoid = Cosmos.Core.Global.BaseIOGroups.TextScreen.Data1.Byte;
            ...
            for(; xPort < 32; xPort++)
            {
                if ((xImplementedPort & 1) != 0)
                {
                    ...
                    if (PortType == PortType.SATA) // If Port type was SATA.
                    {
                        ...
                        var xSATAPort = new Ports.SATA(xPortReg);
