...
using System.Text;
//using Medli.Shell.cmdIntr;
using Medli.System.Utils;
using Cosmos.System;
using IL2CPU.API.Attribs;
using Sys = System;
using Medli;
//using Medli.System.Shell.cmdIntr;
...
namespace MedliPlugs
{
    ...
    public static class ConsoleImpl
    {
        ...
        private static Medli.System.AConsole.Console GetConsole()
        {
            return Medli.Kernel.AConsole;
        }
        ...
        public static String ReadLine()
        {
            ...
            while ((current = KeyboardManager.ReadKey()).Key != ConsoleKeyEx.Enter)
            {
                ...
                if (current.Key == ConsoleKeyEx.Backspace) // Backspace
                {
                    ...
                    {
                        Cosmos.HAL.PCSpeaker.Beep((uint) Notes.A4);
                        Cosmos.HAL.Global.PIT.Wait(10);
                        
                        
                        //Medli.Kernel.speaker.Beep();
                    }
                    continue;
                }
                        ...
                        if (CommandsHistory.CHIndex >= 0)
                        {
                            ...
                            foreach (char chr in Command)
                            {
                                ...
                                else
                                {
                                    ...
                                    Medli.Kernel.AConsole.X -= (chars.Count - currentCount) - 1;
                                    currentCount++;
                                }
